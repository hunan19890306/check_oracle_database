...
def get_database_alert1(oracle_base,service_name,hostname,port,username,password):
    ...
    for q in service_name:
        if q.strip():
            ...
            cmd_database_alert1_2 = "/admin/%s/bdump/" % q
            cmd_database_alert1_3 = "alert_%s1.log|grep ORA-" % q
            cmd_database_alert1 = cmd_database_alert1_1 + cmd_database_alert1_2 + cmd_database_alert1_3
...
def get_database_alert2(oracle_base,service_name,hostname,port,username,password):
    ...
    for q in service_name:
        if q.strip():
            ...
            cmd_database_alert2_2 = "/admin/%s/bdump/" % q
            cmd_database_alert2_3 = "alert_%s2.log|grep ORA-" % q
            cmd_database_alert2 = cmd_database_alert2_1 + cmd_database_alert2_2 + cmd_database_alert2_3
...
#ssh连接到节点一取的信息
    ...
    #ip1变量生成
    #cmd_ip1_1 = "cat /etc/hosts|grep %s|" % hostname1
    #cmd_ip1_2 = "grep -v %s-|awk -F ' ' '{print $1}'" % hostname1
    #cmd_ip1 = cmd_ip1_1 + cmd_ip1_2
    #ip1_stdin, ip1_origin, ip1_stderr = ssh_db1.exec_command(cmd_ip1)
    #ip1 = ip1_origin.read()
    #db_name列表生成
    db_name_stdin, db_name_origin, db_name_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;cat /etc/oratab|grep `echo $ORACLE_HOME`|grep -v Backup|grep -v ASM|awk -F ':' '{print $1}'")
    ...
    #oracle_base生成
    oracle_base_stdin, oracle_base_origin, oracle_base_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;echo $ORACLE_BASE")
    oracle_base = oracle_base_origin.read().replace('\n','')
    #oracle_home生成
    oracle_home_stdin, oracle_home_origin, oracle_home_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;echo $ORACLE_HOME")
    oracle_home = oracle_home_origin.read().replace('\n','')
    #grid_home（crs_home）生成
    grid_home_stdin, grid_home_origin, grid_home_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;env|grep CRS|grep HOME|awk -F '=' '{print $2}'")
    ...
    #cluster变量生成
    cluster_stdin, cluster_origin, cluster_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;crs_stat -t")
    ...
    #ocr_backup变量生成
    ocr_backup_stdin, ocr_backup_origin, ocr_backup_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;ocrconfig -showbackup")
    ocr_backup = ocr_backup_origin.read()
    #asm_alert1变量生成
    cmd_asm_alert1 = "tail -200 %s/admin/+ASM/bdump/alert_+ASM1.log|grep ORA-" % oracle_base
    ...
    #listener1变量生成
    listener1_stdin, listener1_origin, listener1_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile;lsnrctl stat")
    listener1 = listener1_origin.read()
    #listener_log1变量生成
    cmd_listener_log1_1 = "tail -1000 %s/network/log/listener.log|grep TNS-;" % oracle_home
    cmd_listener_log1_2 = "tail -1000 %s/network/log/listener_" % oracle_home
    cmd_listener_log1_3 = "%s.log|grep TNS-" % hostname1
    cmd_listener_log1 = cmd_listener_log1_1 + cmd_listener_log1_2 + cmd_listener_log1_3
...
#ssh连接到节点二取的信息    
    ...
    #version变量生成
    version_stdin, version_origin, version_stderr = ssh_db2.exec_command("source /home/oracle/.bash_profile;sqlplus -v |awk -F ' ' '{print $3}'")
    ...
    #asm_alert2变量生成
    cmd_asm_alert2 = "tail -200 %s/admin/+ASM/bdump/alert_+ASM2.log|grep ORA-" % oracle_base
    ...
    #listener2变量生成
    listener2_stdin, listener2_origin, listener2_stderr = ssh_db2.exec_command("source /home/oracle/.bash_profile;lsnrctl stat")
    listener2 = listener2_origin.read()
    #listener_log2变量生成
    cmd_listener_log2_1 = "tail -1000 %s/network/log/listener.log|grep TNS-;" % oracle_home
    cmd_listener_log2_2 = "tail -1000 %s/network/log/listener_" % oracle_home
    cmd_listener_log2_3 = "%s.log|grep TNS-" % hostname2
    cmd_listener_log2 = cmd_listener_log2_1 + cmd_listener_log2_2 + cmd_listener_log2_3
