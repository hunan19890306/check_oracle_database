...
def get_asm(userinfo,host,service_name,sql):
    ...
    asm = connect_db(userinfo,host,service_name[0],sql)
...
def get_database_version(userinfo,host,service_name,sql):
    document = Document(doc_path)
    database_version = connect_db(userinfo,host,service_name[0],sql)
...
def get_database_alert1(oracle_base,service_name,hostname,port,username,password):
    ...
    for q in service_name:
        if q.strip():
            ...
            names["database_alert1_%s" % q] = names["database_alert1_origin_%s" % q].read().decode('GB2312').encode('utf8')
            code1 = chardet.detect(names["database_alert1_%s" % q])
            print code1
            print names["database_alert1_%s" % q]
            p_total = document.add_paragraph()
...
def get_database_alert2(oracle_base,service_name,hostname,port,username,password):
    ...
    for q in service_name:
        if q.strip():
            ...
            names["database_alert2_%s" % q] = names["database_alert2_origin_%s" % q].read().decode('GB2312').encode('utf8')
            code2 = chardet.detect(names["database_alert2_%s" % q])
            print code2
            p_total = document.add_paragraph()
...
def new_doc(userinfo,host,school_name,author,frequency,ip1,ip2,port1,port2,username1,username2,password1,password2,time,db_name,db_name_output,oracle_base,cpu1,cpu2,memory1,memory2,swap1,swap2,gateway1,gateway2,version,os_version,timezone1,timezone2,syslog1,syslog2,filesystem1,filesystem2,cluster,cluster_alert1,cluster_alert2,crsdlog1,crsdlog2,cssdlog1,cssdlog2,ocr_backup,asm_sql,asm_alert1,asm_alert2,database_version_sql,sga_sql,controlfile_sql,redolog_sql,tablespace_sql,virus_sql,listener1,listener2,listener_log1,listener_log2,doc_path):#新建一个word文档,写入汇总表的数据
    ...
    document.add_paragraph(u'本次总共对%s' % school_name + u'%s套Oracle数据库进行巡检。' % (len(db_name)-1),style='Normal')
...
#输出节点一操作系统文件系统使用率
    ...
    r_total.font.bold = True
    filesystem1_output = document.add_table(1,6,style="Medium Grid 3 Accent 1")
...
#输出节点二操作系统文件系统使用率
    ...
    r_total.font.bold = True
    filesystem2_output = document.add_table(1,6,style="Medium Grid 3 Accent 1")
...
#ssh连接到节点一取的信息
    ...
    #cpu1变量生成
    cpu1_stdin, cpu1_origin, cpu1_stderr = ssh_db1.exec_command("cat /proc/cpuinfo |grep processor |wc -l")
    cpu1 = cpu1_origin.read().replace('\n','')
    #memory1变量生成
    memory1_stdin, memory1_origin, memory1_stderr = ssh_db1.exec_command("cat /proc/meminfo|grep MemTotal|awk -F ':' '{print $2}'|awk -F ' ' '{print $1}'")
    memory1 = memory1_origin.read()
    #swap1变量生成
    swap1_stdin, swap1_origin, swap1_stderr = ssh_db1.exec_command("cat /proc/meminfo|grep SwapTotal|awk -F ':' '{print $2}'|awk -F ' ' '{print $1}'")
    swap1 = swap1_origin.read()
    #gateway1变量生成
    gateway1_stdin, gateway1_origin, gateway1_stderr = ssh_db1.exec_command("netstat -rn|grep UG|awk -F ' ' '{print $2}'")
    gateway1 = gateway1_origin.read().replace('\n','')
    #db_name列表生成
    db_name_stdin, db_name_origin, db_name_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile > /dev/null 2>&1;cat /etc/oratab|grep `echo $ORACLE_HOME`|grep -v Backup|awk -F ':' '{print $1}'")
    ...
    #oracle_base生成
    oracle_base_stdin, oracle_base_origin, oracle_base_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile > /dev/null 2>&1;echo '/u01/app/oracle'")
    oracle_base = oracle_base_origin.read().replace('\n','')
    print oracle_base
    #oracle_home生成
    oracle_home_stdin, oracle_home_origin, oracle_home_stderr = ssh_db1.exec_command("source /home/oracle/.bash_profile > /dev/null 2>&1;echo $ORACLE_HOME")
    oracle_home = oracle_home_origin.read().replace('\n','')
    #grid_base生成
    grid_base_stdin, grid_base_origin, grid_base_stderr = ssh_db1.exec_command("source /home/grid/.bash_profile;echo '/u01/app/grid'")
    grid_base = grid_base_origin.read().replace('\n','')
    #grid_base = '/u01/app/grid'
    #grid_home生成
    grid_home_stdin, grid_home_origin, grid_home_stderr = ssh_db1.exec_command("source /home/grid/.bash_profile > /dev/null 2>&1;echo $ORACLE_HOME")
    grid_home = grid_home_origin.read().replace('\n','')
    #timezone1变量生成
    timezone1_stdin, timezone1_origin, timezone1_stderr = ssh_db1.exec_command("date '+%Y-%m-%d %H:%M:%S';cat /etc/sysconfig/clock|grep ZONE")
    timezone1 = timezone1_origin.read()
    #syslog1变量生成
    syslog1_stdin, syslog1_origin, syslog1_stderr = ssh_db1.exec_command("tail -200 /var/log/messages")
    syslog1 = syslog1_origin.read()
    #filesystem1变量生成
    filesystem1_stdin, filesystem1_origin, filesystem1_stderr = ssh_db1.exec_command("df -h")
    filesystem1 = filesystem1_origin.read().split()
    #cluster变量生成
    cluster_stdin, cluster_origin, cluster_stderr = ssh_db1.exec_command("source /home/grid/.bash_profile > /dev/null 2>&1;crsctl stat res -t")
    cluster = cluster_origin.read()
    #cluster_alert1变量生成
    cmd_cluster_alert1_1 = "tail -200 %s/diag/crs/" % grid_base
    cmd_cluster_alert1_2 = "%s/crs/trace/alert.log" % hostname1
    cmd_cluster_alert1 = cmd_cluster_alert1_1 + cmd_cluster_alert1_2
    ...
    #crsdlog1变量生成
    cmd_crsdlog1_1 = "tail -200 %s" % grid_base
    cmd_crsdlog1_2 = "/diag/crs/%s/crs/trace/crsd.trc" % hostname1
    ...
    #cssdlog1变量生成
    cmd_cssdlog1_1 = "tail -200 %s" % grid_base
    cmd_cssdlog1_2 = "/diag/crs/%s/crs/trace/ocssd.trc" % hostname1
    ...
    #ocr_backup变量生成
    ocr_backup_stdin, ocr_backup_origin, ocr_backup_stderr = ssh_db1.exec_command("source /home/grid/.bash_profile > /dev/null 2>&1;ocrconfig -showbackup")
    ...
    print cmd_asm_alert1
    asm_alert1 = asm_alert1_origin.read()
    #listener1变量生成
    listener1_stdin, listener1_origin, listener1_stderr = ssh_db1.exec_command("source /home/grid/.bash_profile > /dev/null 2>&1;lsnrctl stat")
...
#ssh连接到节点二取的信息    
    ...
    #cpu2变量生成
    cpu2_stdin, cpu2_origin, cpu2_stderr = ssh_db2.exec_command("cat /proc/cpuinfo |grep processor |wc -l")
    cpu2 = cpu2_origin.read().replace('\n','')
    #memory2变量生成
    memory2_stdin, memory2_origin, memory2_stderr = ssh_db2.exec_command("cat /proc/meminfo|grep MemTotal|awk -F ':' '{print $2}'|awk -F ' ' '{print $1}'")
    memory2 = memory2_origin.read()
    #swap2变量生成
    swap2_stdin, swap2_origin, swap2_stderr = ssh_db2.exec_command("cat /proc/meminfo|grep SwapTotal|awk -F ':' '{print $2}'|awk -F ' ' '{print $1}'")
    swap2 = swap2_origin.read()
    #gateway2变量生成
    gateway2_stdin, gateway2_origin, gateway2_stderr = ssh_db2.exec_command("netstat -rn|grep UG|awk -F ' ' '{print $2}'")
    gateway2 = gateway2_origin.read().replace('\n','')
    #version变量生成
    version_stdin, version_origin, version_stderr = ssh_db2.exec_command("source /home/oracle/.bash_profile > /dev/null 2>&1;sqlplus -v |awk -F ' ' '{print $3}'")
    version = version_origin.read()
    #os_version变量生成
    os_version_stdin, os_version_origin, os_version_stderr = ssh_db2.exec_command("cat /etc/issue|grep -v Kernel;uname -r")
    os_version = os_version_origin.read().split('\n')
    #timezone2变量生成
    timezone2_stdin, timezone2_origin, timezone2_stderr = ssh_db2.exec_command("date '+%Y-%m-%d %H:%M:%S';cat /etc/sysconfig/clock|grep ZONE")
    timezone2 = timezone2_origin.read()
    #syslog2变量生成
    syslog2_stdin, syslog2_origin, syslog2_stderr = ssh_db2.exec_command("tail -200 /var/log/messages")
    syslog2 = syslog2_origin.read()
    #filesystem2变量生成
    filesystem2_stdin, filesystem2_origin, filesystem2_stderr = ssh_db2.exec_command("df -h")
    filesystem2 = filesystem2_origin.read().split()
    #cluster_alert2变量生成
    cmd_cluster_alert2_1 = "tail -200 %s/diag/crs/" % grid_base
    cmd_cluster_alert2_2 = "%s/crs/trace/alert.log" % hostname2
    cmd_cluster_alert2 = cmd_cluster_alert2_1 + cmd_cluster_alert2_2
    ...
    #crsdlog2变量生成
    cmd_crsdlog2_1 = "tail -200 %s" % grid_base
    cmd_crsdlog2_2 = "/diag/crs/%s/crs/trace/crsd.trc" % hostname2
    ...
    #cssdlog2变量生成
    cmd_cssdlog2_1 = "tail -200 %s" % grid_base
    cmd_cssdlog2_2 = "/diag/crs/%s/crs/trace/ocssd.trc" % hostname2
    ...
    print cmd_asm_alert2
    asm_alert2 = asm_alert2_origin.read()
    #listener2变量生成
    listener2_stdin, listener2_origin, listener2_stderr = ssh_db2.exec_command("source /home/grid/.bash_profile > /dev/null 2>&1;lsnrctl stat")
