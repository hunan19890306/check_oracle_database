...
#ssh连接到节点一取的信息
    ...
    #db_name列表生成
    db_name_stdin, db_name_origin, db_name_stderr = ssh_db1.exec_command(". /home/oracle/.profile;cat /etc/oratab|grep `echo $ORACLE_HOME`|grep -v Backup|grep -v ASM|awk -F ':' '{print $1}'")
    ...
    #oracle_base生成
    oracle_base_stdin, oracle_base_origin, oracle_base_stderr = ssh_db1.exec_command(". /home/oracle/.profile;echo $ORACLE_BASE")
    oracle_base = oracle_base_origin.read().replace('\n','')
    #oracle_home生成
    oracle_home_stdin, oracle_home_origin, oracle_home_stderr = ssh_db1.exec_command(". /home/oracle/.profile;echo $ORACLE_HOME")
    oracle_home = oracle_home_origin.read().replace('\n','')
    #grid_home(crs_home)生成
    grid_home_stdin, grid_home_origin, grid_home_stderr = ssh_db1.exec_command(". /home/oracle/.profile;env|grep CRS|grep HOME|awk -F '=' '{print $2}'")
    ...
    #cluster变量生成
    cluster_stdin, cluster_origin, cluster_stderr = ssh_db1.exec_command(". /home/oracle/.profile;crs_stat -t")
    ...
    #ocr_backup变量生成
    ocr_backup_stdin, ocr_backup_origin, ocr_backup_stderr = ssh_db1.exec_command(". /home/oracle/.profile;ocrconfig -showbackup")
    ...
    #listener1变量生成
    listener1_stdin, listener1_origin, listener1_stderr = ssh_db1.exec_command(". /home/oracle/.profile;lsnrctl stat")
...
#ssh连接到节点二取的信息    
    ...
    #version变量生成
    version_stdin, version_origin, version_stderr = ssh_db2.exec_command(". /home/oracle/.profile;sqlplus -v |awk -F ' ' '{print $3}'")
    ...
    #listener2变量生成
    listener2_stdin, listener2_origin, listener2_stderr = ssh_db2.exec_command(". /home/oracle/.profile;lsnrctl stat")
